    ...
    \begin{fact}
        \begin{itemize}
            \item Both $\mathcal M$ and $\widehat{\mathcal M}$ are \textbf{column-stochastic} (sums of each column are 1), positive.
                $$
                \sum_i \widehat{M}_{ij} = 1 \  (\forall j) \Longleftrightarrow \boldsymbol{1}^\mathsf T \widehat{M} = \boldsymbol{1}^\mathsf T
                $$
                ($\boldsymbol{1}^\mathsf T$ is a left eigenvector with eigenvalue $1$ $\Rightarrow$ $1$ is an eigenvalue of $\widehat{\mathcal M}$)
            \item Both $\mathcal M$ and $\widehat{\mathcal M}$ are positive matrices. 
        \end{itemize}
    \end{fact}
    By the \textbf{Perron–Frobenius theorem},
    \begin{itemize}
        \item all other eigenvalues $\lambda_i$ satisfy $|\lambda_i| < 1$,
        \item the corresponding eigenvector r has all positive entries.
    \end{itemize}
\end{frame}
...
\begin{frame}{A View on Probability}
    ...
    Reason to use the damping factor? \pause To avoid trapping (jumping across several pages forever). 
    \pause
    \begin{itemize}
    ...
    $$
        \begin{aligned}
            \mathbb P(\text{arriving at $v$ | arriving at $u$}) :=& (1 - d)\frac{1}{n} + \\
            & d\sum_{\text{edge $u\to v$}} \frac{1}{\deg^+(u)}\mathbb P(\text{arriving at $u$})
        \end{aligned}
    $$
\end{frame}
...
\begin{frame}{Further Reading I}
    ...
    \begin{block}{Topic}
        PageRank is actually a variation of \textbf{the Markov Chains}.
    \end{block}
    \begin{fact}
        Random walk which is
        \begin{itemize}
            \item \textbf{irreducible}: with probability of $1/d$, surfer can reach any node with one step $\Rightarrow$ all nodes are reachable ($\Pr > 0$) to the surfer.
            ...
            \item \textbf{positive recurrent}: expected return time to any node is finite
        \end{itemize}
